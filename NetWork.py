...
class batch_norm_relu_layer(nn.Module):
    ...
    """
    def __init__(self, num_features, eps=4e-5, momentum=0.995) -> None:
...
class standard_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        if projection_shortcut is not None:
            self.proj = nn.Sequential(
                nn.Conv2d(in_channels=first_num_filters,out_channels=filters,
                kernel_size=projection_shortcut,stride=strides,padding=0),
                nn.BatchNorm2d(num_features=filters,eps=4e-4,momentum=0.995)
            )
        else:
        ...
        self.l1 = nn.Sequential(
            nn.Conv2d(in_channels=first_num_filters,out_channels=filters,
                kernel_size=(3,3),stride=strides,
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ### YOUR CODE HERE
        projection = self.proj(inputs)
        l1 = self.l1(inputs)
        l2 = self.l2(l1)
        outputs = torch.add(projection,l2)
...
class bottleneck_block(nn.Module):
    ...
    def __init__(self, filters, projection_shortcut, strides, first_num_filters) -> None:
        ...
        self.l1 = nn.Sequential(
            batch_norm_relu_layer(num_features=first_num_filters),
            nn.Conv2d(
                in_channels=first_num_filters,out_channels=filters//4,
                kernel_size=(1,1),stride=strides,
                padding=0 if strides>1 else 'same'
            ))
        self.l2 = nn.Sequential(
            batch_norm_relu_layer(num_features=filters//4),
            nn.Conv2d(in_channels=filters//4,out_channels=filters//4,kernel_size=(3,3),
                     stride=1,padding='same'))
        self.l3 = nn.Sequential(
            batch_norm_relu_layer(num_features=filters//4),
            nn.Conv2d(in_channels=filters//4,out_channels=filters,
...
class stack_layer(nn.Module):
    ...
    def __init__(self, filters, block_fn, strides, resnet_size, first_num_filters) -> None:
        ...
        in_ch=filters_out//2 if strides>1 else filters
        proj_shortcut=(1,1) if strides>1 or block_fn is bottleneck_block else None
        self.stack.append(block_fn(filters=filters_out,projection_shortcut= proj_shortcut,
                         strides=strides,first_num_filters=in_ch))
...
class output_layer(nn.Module):
    ...
    def __init__(self, filters, resnet_version, num_classes) -> None:
        ...
        if (resnet_version == 2):
            self.bn_relu = batch_norm_relu_layer(filters, eps=4e-5, momentum=0.995)
        
        ### END CODE HERE
        self.avg_pooling = nn.Sequential(nn.AdaptiveAvgPool2d((1,1)),nn.Flatten())
        in_feat = filters if resnet_version==2 else filters//4
        self.fc_layer = nn.Linear(in_features=in_feat,out_features=num_classes)
        ### END CODE HERE
    
    def forward(self, inputs: Tensor) -> Tensor:
        ### END CODE HERE
        outputs = self.avg_pooling(inputs)
